...
Page storage and Export to Markdown should share the same renderer:

1. Build an export document with [`createExportDocument`](../../src/lib/visual-note/export/document.ts).
2. Parse and serialize article content through [`parseArticleContent`](../../src/lib/visual-note/article-content.ts) and [`serializeArticleContent`](../../src/lib/visual-note/article-content.ts).
3. Render markdown with [`renderMarkdownExport`](../../src/lib/visual-note/export/markdown.ts).
...
Page article content itself should not be stored as JSON schemas. See
[notebook-storage.md](./notebook-storage.md) for the page markdown storage
requirements.
...
- Reuse Export to Markdown formatting
...
6. Verify Export to Markdown and page storage use the same rendered markdown for equivalent page scope.
7. Verify `GET /api/pages/[pageId]/content` returns markdown, not a JSON block schema.
